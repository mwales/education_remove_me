...
void loop()
{
   ...
   float smallLoadVoltage, largeLoadVoltage;   // Stores the voltage value we need to print for user
   ...
   smallLoadVoltage = (float) sensorRaw * scalarValue;
   ...
   // Convert the raw sensor value into human-readable voltage
   largeLoadVoltage = (float) sensorRaw * scalarValue;

   Serial.print(", ");
   Serial.print(boardName);
   Serial.print(", Small Load, ");
   Serial.print(smallLoadVoltage, 2);
   Serial.print(", Large Load, ");
   Serial.println(largeLoadVoltage, 2);
         
   if (handleInputAndDelay(30))
   {
     processInput();
   }
