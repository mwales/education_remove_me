...
void ConwayCube::runLifeCycle()
{
	...
	for(int x = theMinC.x - 1; x <= theMaxC.x + 1; x++)
	{
		for(int y = theMinC.y - 1; y <= theMaxC.y + 1; y++)
		{
			...
			{
				for(int w = theMinC.w - 1; w <= theMaxC.w + 1; w++)
				{
					Coord c = {x,y,z,w};
