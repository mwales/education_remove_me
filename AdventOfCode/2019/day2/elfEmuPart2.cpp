...
int main(int argc, char** argv)
{
	while(true)
	{
		...
		// Day 2 special stuff
		// Find what 1 and 2 have to be for the answer to be 19690720
		...
		for(int pos1 = 0; pos1 <= 99; pos1++)
		{
			for(int pos2 = 0; pos2 <= 99; pos2++)
			{
				...
				std::vector<int> endData = runProgram(programData);

				if (solution == endData[0])
				{
					std::cout << "Solution: pos[1]=" << pos1 << ", and pos[2]=" << pos2 << std::endl;
					std::cout << "Solution: NV=" << pos1 * 100 + pos2 << std::endl;
					goto escape_loop;
				}
			}
		}
