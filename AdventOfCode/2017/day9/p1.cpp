...
// To use the samples, you have to split them
//   split -l 1 samples.txt
// And then run them 1 at a time
//   ls sampleDir/x?? | xargs -L 1 ./a.out
// Then remove the split files
//   rm x??
...
things, separated by commas: either another group or garbage. Since groups can contain other groups,
a } only closes the most-recently-opened unclosed group - that is, they are nestable. Your puzzle
input represents a single, large group which itself contains many smaller ones.
...
int main(int argc, char** argv)
{
   ...
   int garbageThrownOut = 0;

   bool garbageState = false;
   bool throwAwayState = false;
   ...
   for(int i = 0; i < text.length(); i++)
   {
      ...
      if (curChar == '!')
      {
         if (!garbageState)
         {
            std::cerr << "Found an invalid ! at byte " << i << std::endl;
            break;
         }
   ...
   std::cout << "Garbage Removed = " << garbageThrownOut << std::endl;
